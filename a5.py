...
class Board:
    ...
    def find_most_constrained_cell(self) -> Tuple[int, int]:
        ...
        for row in range(0,9):
            for column in range(0,9):
                if not(isinstance(self.rows[row][column], int)):
                    if is_first:
                        current_smallest = (row, column)
                        is_first = False
                    if len(self.rows[row][column]) < len(self.rows[current_smallest[0]][current_smallest[1]]):
                        current_smallest = (row, column)
    ...
    def update(self, row: int, column: int, assignment: int) -> None:
        ...
        #remove all possibilities in 3x3 square
        for coordinates in self.subgrid_coordinates(row, column):
            remove_if_exists(self.rows[coordinates[0]][coordinates[1]], assignment)
           
        #remove all possibilities in rows and columns
        for i in range(0,9):
            for k in range(0,9):
                if i == row or k == column:
                    remove_if_exists(self.rows[i][k], assignment)
...
def DFS(state: Board) -> Board:
    ...
    while s.is_empty() == False:
        ...
        if cur_b.goal_test():
            return cur_b
        elif cur_b.failure_test() == False:
            
            for i in cur_b.rows[cur_b.find_most_constrained_cell()[0]][cur_b.find_most_constrained_cell()[1]]:
                
                new_board = copy.deepcopy(cur_b)
                new_board.update(cur_b.find_most_constrained_cell()[0], cur_b.find_most_constrained_cell()[1], i)
                s.push(new_board)
...
def test():
    for i in isinstance((0,1,4),tuple):
        print(i)
